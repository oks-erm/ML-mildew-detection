...
The hyperparameters of the model are optimized using Keras Tuner, which is a hyperparameter tuning framework that allows automatic tuning of hyperparameters using various search algorithms. The hyperparameters tuned in this model are the number of units in the dense layer and the learning rate of the optimizer. The optimal number of units in the dense layer is determined by the hp.Int method, which searches for the optimal number of units between 16 and 64 in increments of 16. The optimal learning rate is determined by the hp.Choice method, which selects the learning rate from a set of predefined values.

To prevent overfitting, a dropout layer with a rate of 0.5 is added after the dense layer. tanh activation function is used for the output layer, which is a good alternative to sigmoid for binary classification tasks. The learning rate used for training the network is also a hyperparameter, which is selected by the hp_learning_rate variable using the hp.Choice method. The Adam optimizer is used to minimize the binary cross-entropy loss function.
...
The model met the requirements of the project, but had relatively low recall score. Relatively high rate of infected leaves misidentified as healthy made me attempt to minimise this metric.
...
whilst the performance metrics on training set were superior to the previous iteration, including lower losses.
...
2. Log into Heroku CLI in IDE workspace terminal using the bash command: `heroku login -i` and enter credentials.
...
* Open your IDE of your choice and change the current working directory to the location where you want the cloned directory to be located.
...
- [Heroku](https://www.heroku.com/) - Deployment of dashboard as a web application
...
* Code Institute Malaria Walk Through Project was used for instructional purposes, and guidance through the development of this project. For reference and organisation of the app, even though I attempted to optimise the code provided to avoid just copying.
...
- The images used for the design are generated by Stable Difussion.
____
